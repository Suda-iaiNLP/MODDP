...
def orthonormal_initializer(output_size, input_size):
    ...
    """
    logger.info(f"output_size: {output_size}, input_size: {input_size}")
    ...
    while not success and tries < 10:
        ...
        success = True
    if success:
        logger.info("Orthogonal pretrainer loss: %.2e" % loss)
    else:
        logger.info("Orthogonal pretrainer failed, using non-orthogonal random matrix")
        Q = np.random.randn(input_size, output_size) / np.sqrt(output_size)
...
def pad_sequence(xs, length=None, padding=-1, dtype=np.float64):
    ...
    if length is None:
        length = max(lengths)
    y = np.array([np.pad(x.astype(dtype), (0, length - l), mode="constant", constant_values=padding) for x, l in zip(xs, lengths)])
    return torch.from_numpy(y)


def get_extended_attention_mask(attention_mask, input_shape, dtype=torch.float32):
    """
